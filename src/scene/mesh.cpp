...
void Mesh::createCube(const char *textureFile, const char *bgTextureFile)
{
    ...
    std::vector<glm::vec2> cubeUvs {glm::vec2(0, 0),
                                     glm::vec2(1, 0),
                                     glm::vec2(1, 1),
                                     glm::vec2(0, 1)};
    // add front, back, left, right, top, bottom UVs into UVs
    std::vector<glm::vec2> uvs = cubeUvs;
    for(int i = 0; i < 5; i++)
    {
        uvs.insert(uvs.end(), cubeUvs.begin(), cubeUvs.end());
    }
    ...
    // TODO: Set "count" to the number of indices in your index VBO
    count = idx.size();
